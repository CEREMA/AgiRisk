...
CREATE OR REPLACE FUNCTION public.__indic_s3_1a_rc(nom_ter text, typ_alea text, code_occ text, an_bdtopo text)
 RETURNS void
 LANGUAGE plpgsql
AS $function$

-- Fonction SQL de représentation cartographique de l'indicateur S3/1a "Occupants en zone inondable"
-- © Cerema / GT AgiRisk (auteurs du script : Anaïs (idée originale pour le croisement avec les hexagones), Lucie (optimisation du croisement avec les hexagones et avec les entités administratives), Sébastien (trame de départ puis mises à jour dans version 2))
...
-- SELECT public.__indic_s3_1a_rc('Jura', 'débordement de cours d''eau', 'QRef', '2022');
...
DECLARE
	...
	typ_res varchar; -- variable qui prend la valeur du type_result à insérer dans s3_1a_rc
...
BEGIN
	...
	THEN
		RAISE EXCEPTION 'L''indicateur S3/1a n''a pas encore été calculé sur le territoire "%" avec l''aléa "% - %". Fin de l''exécution', nom_ter, typ_alea, code_occ;
	END IF;

	--************************************************************************
	-- Vérification de l'absence de données pour le territoire, le type d'aléa et l'occurrence de crue renseignés dans la table p_rep_carto.s3_1a_rc
	--************************************************************************
	IF EXISTS (
		SELECT *
		FROM p_rep_carto.s3_1a_rc
		WHERE territoire = ''||nom_ter||''
			AND type_alea = ''||typ_alea||''
			AND code_occurrence = ''||code_occ||''
	)
	THEN
		RAISE EXCEPTION 'Il existe déjà des enregistrements pour le territoire "%" avec l''aléa "% - %" dans la table p_rep_carto.s3_1a_rc. 
		Pour les supprimer, lancer la requête suivante : 
		DELETE FROM p_rep_carto.s3_1a_rc WHERE territoire = ''%'' AND type_alea = ''%'' AND code_occurrence = ''%'';', nom_ter, typ_alea, code_occ, REPLACE(nom_ter,'''',''''''), REPLACE(typ_alea,'''',''''''), REPLACE(code_occ,'''','''''');
	END IF;
	...
	LOOP
		...
		SELECT DISTINCT
			...
			CEIL(COALESCE(SUM(s3_1a.pop2_bas) FILTER (WHERE s3_1a.loc_zx = ''In''),0)),
			CEIL(COALESCE(SUM(s3_1a.pop2_haut) FILTER (WHERE s3_1a.loc_zx = ''Out''),0)),
		...
		FROM s3_1a
		JOIN niv_zoom
		ON right(s3_1a.moda_calc,5) = niv_zoom.code_indic
		...
		GROUP BY s3_1a.territoire, s3_1a.type_alea, s3_1a.code_occurrence, type_result, id_geom, s3_1a.date_calc, hexag.geom
		';
	END LOOP;
	...
	EXECUTE 'INSERT INTO s3_1a_rc (
		...
		WHERE __util_to_snake_case(zt.territoire) = '''||__util_to_snake_case(nom_ter)||'''
			AND __util_to_snake_case(s3_1a.territoire) = '''||__util_to_snake_case(nom_ter)||'''
			AND __util_to_snake_case(s3_1a.type_alea) = '''||__util_to_snake_case(typ_alea)||'''
			AND __util_to_snake_case(s3_1a.code_occurrence) = '''||__util_to_snake_case(code_occ)||'''
			AND niv_zoom.niv_zoom = 0
		GROUP BY s3_1a.territoire, s3_1a.type_alea, s3_1a.code_occurrence, niv_zoom.type_result, zt.id_iris, s3_1a.nom_iris, s3_1a.loc_zx, s3_1a.date_calc
		';
	...
	EXECUTE 'DROP TABLE IF EXISTS s3_1a_iris CASCADE;
		CREATE TEMP TABLE s3_1a_iris AS
		SELECT
			niv_zoom.type_result,
			s3_1a.id_iris,
			s3_1a.nom_iris,
			...
			CEIL(COALESCE(SUM(s3_1a.pop2_bas) FILTER (WHERE s3_1a.loc_zx = ''In''),0)) AS pop2_bas_in,
			CEIL(COALESCE(SUM(s3_1a.pop2_haut) FILTER (WHERE s3_1a.loc_zx = ''Out''),0)) AS pop2_haut_out,
		...
		GROUP BY s3_1a.territoire, s3_1a.type_alea, s3_1a.code_occurrence, niv_zoom.type_result, s3_1a.id_iris, s3_1a.nom_iris, s3_1a.date_calc, s3_1a.moda_calc
		';
	
	EXECUTE 'CREATE INDEX ON s3_1a_iris USING btree(id_iris)';

	-- Il est IMPÉRATIF d'enregistrer la valeur du champ type_result dans une variable déclarée "typ_res" car l'appel au champ s3_1a_iris.type_result dans l'INSERT INTO ci-dessous fait sauter les IRIS non concernés par des parcelles s3_1a. Ce dysfonctionnement ressemble fortement à un bug PostgreSQL.
	...
	typ_res := (SELECT DISTINCT type_result FROM s3_1a_iris);
	...
	EXECUTE 'INSERT INTO s3_1a_rc (
		...
		SELECT
			...
			zt.id_iris,
			s3_1a.nom_iris,
			''indéterminé''::varchar(30),
			s3_1a.pop1_in, s3_1a.pop1_out,
			s3_1a.pop2_haut_in, s3_1a.pop2_bas_in, s3_1a.pop2_haut_out, s3_1a.pop2_bas_out,
			s3_1a.pop3_in, s3_1a.pop3_out,
			s3_1a.pop4_in, s3_1a.pop4_out,
			s3_1a.pop5_in, s3_1a.pop5_out,
			s3_1a.pop6_haut_in, s3_1a.pop6_bas_in, s3_1a.pop6_haut_out, s3_1a.pop6_bas_out,
			1,
			s3_1a.date_calc,
			zt.geom
		FROM s3_1a_iris as s3_1a
	...
	EXECUTE 'DROP TABLE IF EXISTS s3_1a_com CASCADE;
		CREATE TEMP TABLE s3_1a_com AS
		SELECT
			niv_zoom.type_result,
			s3_1a.id_commune,
			s3_1a.nom_commune,
		...
		GROUP BY s3_1a.territoire, s3_1a.type_alea, s3_1a.code_occurrence, niv_zoom.type_result, s3_1a.id_commune, s3_1a.nom_commune, s3_1a.date_calc, s3_1a.moda_calc
		';
	
	EXECUTE 'CREATE INDEX ON s3_1a_com USING btree(id_commune)';

	-- Il est IMPÉRATIF d'enregistrer la valeur du champ type_result dans une variable déclarée "typ_res" car l'appel au champ s3_1a_com.type_result dans l'INSERT INTO ci-dessous fait sauter les communes non concernées par des parcelles s3_1a. Ce dysfonctionnement ressemble fortement à un bug PostgreSQL.
	-- Lucie a absolument TOUT tenté pour éviter de passer par cette variable déclarée sans succès. Cette méthode ajoute simplement deux lignes à l'ensemble du code, mais à creuser plus tard éventuellement.
	typ_res := (SELECT DISTINCT type_result FROM s3_1a_com);
	...
	THEN
		...
		EXECUTE 'DROP TABLE IF EXISTS s3_1a_epci CASCADE;
			CREATE TEMP TABLE s3_1a_epci AS
			SELECT
				niv_zoom.type_result,
				s3_1a.id_epci,
				s3_1a.nom_epci,
			...
			GROUP BY s3_1a.territoire, s3_1a.type_alea, s3_1a.code_occurrence, niv_zoom.type_result, s3_1a.id_epci, s3_1a.nom_epci, s3_1a.date_calc, s3_1a.moda_calc
			';
	
		EXECUTE 'CREATE INDEX ON s3_1a_epci USING btree(id_epci)';
	
		-- Il est IMPÉRATIF d'enregistrer la valeur du champ type_result dans une variable déclarée "typ_res" car l'appel au champ s3_1a_epci.type_result dans l'INSERT INTO ci-dessous fait sauter les EPCI non concernés par des parcelles s3_1a. Ce dysfonctionnement ressemble fortement à un bug PostgreSQL.
		-- Lucie a absolument TOUT tenté pour éviter de passer par cette variable déclarée sans succès. Cette méthode ajoute simplement deux lignes à l'ensemble du code, mais à creuser plus tard éventuellement.
		typ_res := (SELECT DISTINCT type_result FROM s3_1a_epci);
	...
	ELSE
		...
		EXECUTE 'DROP TABLE IF EXISTS s3_1a_ter CASCADE;
			CREATE TEMP TABLE s3_1a_ter AS
			SELECT
				s3_1a.territoire,
				niv_zoom.type_result,
				s3_1a.date_calc,
			...
			WHERE __util_to_snake_case(s3_1a.territoire) = ''' ||__util_to_snake_case(nom_ter)|| '''
				...
				AND __util_to_snake_case(s3_1a.code_occurrence) = '''||__util_to_snake_case(code_occ)||'''
				AND niv_zoom.niv_zoom = 8
			GROUP BY s3_1a.territoire, s3_1a.type_alea, s3_1a.code_occurrence, niv_zoom.type_result, s3_1a.date_calc, s3_1a.moda_calc
			';

		EXECUTE 'CREATE INDEX ON s3_1a_ter USING btree(territoire)';
	
		-- Il est IMPÉRATIF d'enregistrer la valeur du champ type_result dans une variable déclarée "typ_res" car l'appel au champ s3_1a_ter.type_result dans l'INSERT INTO ci-dessous fait sauter les EPCI non concernés par des parcelles s3_1a. Ce dysfonctionnement ressemble fortement à un bug PostgreSQL.
		-- Lucie a absolument TOUT tenté pour éviter de passer par cette variable déclarée sans succès. Cette méthode ajoute simplement deux lignes à l'ensemble du code, mais à creuser plus tard éventuellement.
		typ_res := (SELECT DISTINCT type_result FROM s3_1a_ter);
		...
		EXECUTE 'INSERT INTO s3_1a_rc (
			...
			SELECT DISTINCT
				...
				s3_1a.date_calc,
				ter.geom
			FROM s3_1a_ter AS s3_1a
			RIGHT JOIN ter_temp AS ter
			ON __util_to_snake_case(s3_1a.territoire) = __util_to_snake_case(ter.territoire)
	...
	--************************************************************************
	-- Création de la vue matérialisée s3_1a_rc sur le territoire
	--************************************************************************
	RAISE NOTICE '';
	RAISE NOTICE 'Création de la vue matérialisée %', 'p_rep_carto.s3_1a_rc_'||__util_to_snake_case(nom_ter);
	EXECUTE '
		DROP MATERIALIZED VIEW IF EXISTS s3_1a_rc_'||__util_to_snake_case(nom_ter)||' CASCADE;
	...
	EXECUTE 'CREATE UNIQUE INDEX ON s3_1a_rc_' || __util_to_snake_case(nom_ter) || ' (id)';
	...
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(pct1_in)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(pct1_out)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(total1)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(pct2_haut_in)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(pct2_bas_in)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(pct2_haut_out)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(pct2_bas_out)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(total2_haut)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(total2_bas)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(pct3_in)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(pct3_out)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(total3)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(pct4_in)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(pct4_out)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(total4)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(pct5_in)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(pct5_out)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(total5)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(pct6_haut_in)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(pct6_bas_in)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(pct6_haut_out)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(pct6_bas_out)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(total6_haut)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(total6_bas)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(valid_result)';
	EXECUTE 'CREATE INDEX ON s3_1a_rc_'||__util_to_snake_case(nom_ter)||' USING btree(date_calc)';
	...
	RAISE NOTICE '[INFO] La table s3_1a_rc (représentation carto de l''indicateur sous forme d''hexagones + autres géométries administratives) a été mise à jour dans le schéma p_rep_carto pour le territoire "%" et l''aléa "% - %"', nom_ter, typ_alea, code_occ;
	RAISE NOTICE '';
	RAISE NOTICE '====== RÉSULTATS ======';

	IF c_hexag+c_entite+c_iris+c_com+c_epci > 1
		THEN RAISE NOTICE '% enregistrements insérés au total dans la table p_rep_carto.s3_1a_rc pour le territoire "%" et l''aléa "% - %"', c_hexag+c_entite+c_iris+c_com+c_epci, nom_ter, typ_alea, code_occ;
		ELSE RAISE NOTICE '% enregistrement inséré dans la table p_rep_carto.s3_1a_rc pour le territoire "%" et l''aléa "% - %"', c_hexag+c_entite+c_iris+c_com+c_epci, nom_ter, typ_alea, code_occ;
	END IF;
