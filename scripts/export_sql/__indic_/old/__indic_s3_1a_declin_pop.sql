CREATE OR REPLACE FUNCTION public.__indic_s3_1a_declin_pop(nom_ter text, typ_alea text, code_occ text, an_bdtopo text)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
...
-- Exemple d'appel à cette fonction : SELECT public.__indic_s3_1a_DECLIN_POP('Jura', 'débordement de cours d''''eau', 'QRef', '2022');
...
BEGIN
    ...
    --************************************************************************
    -- Vérification de l'absence dans la table p_indicateurs.s3_1a à la fois du territoire, du type d'aléa et de l'occurrence de crue renseignés
	--************************************************************************
	IF EXISTS(
		SELECT *
		FROM s3_1a
	...
	THEN
		RAISE EXCEPTION 'Il existe déjà des enregistrements pour le territoire ''%'' avec l''aléa ''% - %'' dans la table p_indicateurs.s3_1a. Pour les supprimer, lancer la requête suivante : DELETE FROM p_indicateurs.s3_1a WHERE territoire = ''%'' AND type_alea = ''%'' AND code_occurrence = ''%'';', nom_ter, typ_alea, code_occ, nom_ter, typ_alea, code_occ;
	END IF;
	...
	EXECUTE 'INSERT INTO p_indicateurs.s3_1a(id_bdt, territoire, id_iris, nom_iris, loc_zx, code_occurrence, type_alea, date_calcul, modalite_calcul, pop1, pop2_haut, pop2_bas, pop3, pop4, pop5, pop6_haut, pop6_bas, geom, geomloc)
	SELECT
		oc1.id_bdt,
		oc1.territoire,
		oc1.id_iris,
		...
		''__indic_s3_1a''::varchar(50) AS modalite_calcul,
	...
	-- 2a. Attribution des codes INSEE et noms de communes
		EXECUTE '
			...
			WHERE s3_1a.id_iris = zt.id_iris
				AND zt.id_commune = com.insee_com
				AND s3_1a.territoire = ''' || nom_ter || '''
				AND s3_1a.type_alea = ''' || typ_alea || '''
				AND s3_1a.code_occurrence = ''' || code_occ || ''''; -- conditions ajoutées pour réduire le temps de calcul aux seules entités associées au territoire et à l'aléa renseignés
	...
	RAISE NOTICE '% entités ajoutées pour le territoire ''%'' et l''aléa ''% - %'' dans p_indicateurs.s3_1a', c, nom_ter, typ_alea, code_occ;
