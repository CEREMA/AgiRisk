CREATE OR REPLACE FUNCTION public.__indic_s2_2a(nom_ter text, typ_alea text, code_occ text, an_fch_acb text, an_bdtopo text, an_fct_dmg text)
 RETURNS void
 LANGUAGE plpgsql
AS $function$

-- Fonction SQL d'incrémentation de la table p_indicateurs.s2_2a "Montant des dommages aux logements en fonction de la hauteur d'eau et de la durée de submersion"
...
-- SELECT public.__indic_s2_2a('TRI Verdun', 'débordement de cours d''eau + remontée de nappe', 'QRef diag', '2021', '2022', '2021');
...
BEGIN
    ...
    --************************************************************************
    -- Vérification de l'absence dans la table p_indicateurs.s2_2a à la fois du territoire, du type d'aléa et de l'occurrence de crue renseignés
	--************************************************************************
	IF EXISTS(
		SELECT *
		FROM p_indicateurs.s2_2a
		WHERE territoire = ''||nom_ter||''
			AND type_alea = ''||typ_alea||''
			AND code_occurrence = ''||code_occ||''
	)
	THEN
		RAISE EXCEPTION 'Il existe déjà des enregistrements pour le territoire "%" et l''aléa "% - %" dans la table p_indicateurs.s2_2a. 
		Pour les supprimer, lancer la requête suivante : 
		DELETE FROM p_indicateurs.s2_2a WHERE territoire = ''%'' AND type_alea = ''%'' AND code_occurrence = ''%'';', nom_ter, typ_alea, code_occ, REPLACE(nom_ter,'''',''''''), REPLACE(typ_alea,'''',''''''), REPLACE(code_occ,'''','''''');
	END IF;
	...
	RAISE NOTICE 'Etape 1a : intersection de la couche des logements avec Zh (classes de hauteur d''eau)';
        ...
        EXECUTE '
            WITH lgts_in_zh AS(
                SELECT DISTINCT
	                ...
	                ''__indic_s2_2a''::varchar(50) AS moda_calc,
					oc2.geom AS geom,
					oc2.geomloc AS geomloc
                FROM oc2_amc AS oc2, c_phenomenes.zh
                WHERE ST_Intersects(oc2.geom, zh.geom)
					AND __util_quote_to_snake_case(oc2.territoire) = '''||__util_quote_to_snake_case(nom_ter)||'''
					AND __util_quote_to_snake_case(zh.territoire) = '''||__util_quote_to_snake_case(nom_ter)||'''
					AND __util_quote_to_snake_case(zh.type_alea) = '''||__util_quote_to_snake_case(typ_alea)||'''
					AND __util_quote_to_snake_case(zh.code_occurrence) = '''||__util_quote_to_snake_case(code_occ)||'''
				GROUP BY zh.territoire,
					...
					oc2.geom,
					oc2.geomloc)
            INSERT INTO p_indicateurs.s2_2a(
				territoire,
				loc_zx,
				type_alea,
				code_occurrence,
		...
		EXECUTE '
			UPDATE p_indicateurs.s2_2a AS s2_2a
			SET surf_rdc_rect_alea = ROUND(CAST(surf_rdc_rect_tot * (surf_polygon_alea / surf_polygon_tot) AS numeric),2)
			WHERE __util_quote_to_snake_case(s2_2a.territoire) = '''||__util_quote_to_snake_case(nom_ter)||'''
				AND s2_2a.loc_zx = ''In''
				AND s2_2a.typo_acb <> ''AUTRE''';
        ...
        EXECUTE '
            WITH lgts_out_zx AS(
                ...
                WHERE NOT ST_Intersects(oc2.geom, zx.geom)
					AND __util_quote_to_snake_case(oc2.territoire) = '''||__util_quote_to_snake_case(nom_ter)||'''
					AND __util_quote_to_snake_case(zx.territoire) = '''||__util_quote_to_snake_case(nom_ter)||'''
					AND __util_quote_to_snake_case(zx.type_alea) = '''||__util_quote_to_snake_case(typ_alea)||'''
					AND __util_quote_to_snake_case(zx.code_occurrence) = '''||__util_quote_to_snake_case(code_occ)||'''
	...
	RAISE NOTICE 'Etape 1c : attribution d''un niveau d''aléa arbitraire en fonction de la hauteur d''eau, mais ne tenant pas compte de la dynamique';
		EXECUTE '
			...
			SET intensite_alea = (
				CASE
					WHEN s2_2a.h_eau_max = 0 THEN ''nulle''
					WHEN s2_2a.h_eau_max = 50 THEN ''faible''
					WHEN s2_2a.h_eau_max = 100 THEN ''moyen''
					WHEN s2_2a.h_eau_max = 150 OR s2_2a.h_eau_max = 200 THEN ''fort''
					WHEN s2_2a.h_eau_max = 999 THEN ''très fort''
					ELSE intensite_alea
			END)
			WHERE __util_quote_to_snake_case(s2_2a.territoire) = '''||__util_quote_to_snake_case(nom_ter)||'''
				AND __util_quote_to_snake_case(s2_2a.type_alea) = '''||__util_quote_to_snake_case(typ_alea)||'''
				AND __util_quote_to_snake_case(s2_2a.code_occurrence) = '''||__util_quote_to_snake_case(code_occ)||'''
				AND s2_2a.date_actu_cout_dmg = '''||an_fct_dmg||'''';
        ...
        EXECUTE '
            WITH lgts_in_zq AS(
                SELECT DISTINCT
	                ...
	                oc2.typo_acb AS typo_acb,
					oc2.surf_polygon_tot AS surf_polygon_tot,
					ROUND(CAST(ST_Area(ST_Intersection(oc2.geom, zq.geom)) AS numeric),2) AS surf_polygon_alea,
					oc2.surf_rdc_rect_tot AS surf_rdc_rect_tot,
					oc2.sous_sol AS sous_sol,
					oc2.surf_ssol_tot_bat AS surf_ssol_tot_bat,
					oc2.surf_ssol_rap_logt AS surf_ssol_rap_logt,
                ...
                WHERE ST_Intersects(oc2.geom, zq.geom)
					AND __util_quote_to_snake_case(oc2.territoire) = '''||__util_quote_to_snake_case(nom_ter)||'''
					AND __util_quote_to_snake_case(zq.territoire) = '''||__util_quote_to_snake_case(nom_ter)||'''
					AND __util_quote_to_snake_case(zq.type_alea) = '''||__util_quote_to_snake_case(typ_alea)||'''
					AND __util_quote_to_snake_case(zq.code_occurrence) = '''||__util_quote_to_snake_case(code_occ)||'''
	...
	RAISE NOTICE 'Etape 1c : attribution de hauteurs d''eau arbitraires en fonction de l''intensité de l''aléa, mais ne tenant pas compte de la dynamique';
		EXECUTE '
			...
			SET h_eau_min = (
				CASE
					WHEN s2_2a.intensite_alea = ''nulle'' THEN 0
					WHEN s2_2a.intensite_alea = ''faible'' THEN 0
					WHEN s2_2a.intensite_alea = ''moyen'' THEN 50
					WHEN s2_2a.intensite_alea = ''fort'' THEN 100
					WHEN s2_2a.intensite_alea = ''très fort'' THEN 200
					ELSE h_eau_min
			END)
			WHERE __util_quote_to_snake_case(s2_2a.territoire) = '''||__util_quote_to_snake_case(nom_ter)||'''
				AND __util_quote_to_snake_case(s2_2a.type_alea) = '''||__util_quote_to_snake_case(typ_alea)||'''
		...
		EXECUTE '
			...
			SET h_eau_max = (
				CASE
					WHEN s2_2a.intensite_alea = ''nulle'' THEN 0
					WHEN s2_2a.intensite_alea = ''faible'' THEN 50
					WHEN s2_2a.intensite_alea = ''moyen'' THEN 100
					WHEN s2_2a.intensite_alea = ''fort'' THEN 200
					WHEN s2_2a.intensite_alea = ''très fort'' THEN 999
					ELSE h_eau_max
			END)
			WHERE __util_quote_to_snake_case(s2_2a.territoire) = '''||__util_quote_to_snake_case(nom_ter)||'''
				AND __util_quote_to_snake_case(s2_2a.type_alea) = '''||__util_quote_to_snake_case(typ_alea)||'''
	...
	-- a. Récupération des codes et noms des IRIS
		EXECUTE '
			...
			WHERE ST_Intersects(s2_2a.geom, zt.geom)
				AND s2_2a.date_actu_cout_dmg = '''||an_fct_dmg||'''
				AND __util_quote_to_snake_case(s2_2a.territoire) = '''||__util_quote_to_snake_case(nom_ter)||'''
				AND __util_quote_to_snake_case(s2_2a.type_alea) = '''||__util_quote_to_snake_case(typ_alea)||'''
				AND __util_quote_to_snake_case(s2_2a.code_occurrence) = '''||__util_quote_to_snake_case(code_occ)||''''; -- conditions ajoutées pour réduire le temps de calcul aux seules entités associées au territoire et à l'aléa renseignés
	...
	-- b. Attribution des codes INSEE et noms de communes
		EXECUTE '
			...
			WHERE s2_2a.id_iris = zt.id_iris
				AND zt.id_commune = com.insee_com
				AND s2_2a.date_actu_cout_dmg = '''||an_fct_dmg||'''
				AND __util_quote_to_snake_case(s2_2a.territoire) = '''||__util_quote_to_snake_case(nom_ter)||''''; -- condition ajoutée pour réduire le temps de calcul aux seules entités associées au territoire renseigné
	...
	-- c. Attribution des codes et noms des EPCI
		EXECUTE '
			...
			WHERE s2_2a.id_iris = zt.id_iris
				...
				AND s2_2a.date_actu_cout_dmg = '''||an_fct_dmg||'''
				AND __util_to_snake_case(s2_2a.territoire) = '''||__util_quote_to_snake_case(nom_ter)||''''; -- condition ajoutée pour réduire le temps de calcul aux seules entités associées au territoire renseigné
	...
	RAISE NOTICE 'Etape 3 : attribution d''un nom de type d''aléa conforme à la nomenclature des fonctions de dommages';
		EXECUTE '
			...
			SET type_alea_fct_dmg = (
				CASE
					WHEN s2_2a.type_alea LIKE ''%bordement%'' THEN ''fluvial''
					WHEN s2_2a.type_alea LIKE ''%fluvial%'' THEN ''fluvial''
					WHEN s2_2a.type_alea LIKE ''%ubmersi%'' THEN ''marin''
					WHEN s2_2a.type_alea LIKE ''%marin%'' THEN ''marin''
			END)
			WHERE __util_quote_to_snake_case(s2_2a.territoire) = '''||__util_quote_to_snake_case(nom_ter)||'''
				AND __util_quote_to_snake_case(s2_2a.type_alea) = '''||__util_quote_to_snake_case(typ_alea)||'''
				AND __util_quote_to_snake_case(s2_2a.code_occurrence) = '''||__util_quote_to_snake_case(code_occ)||'''
	...
	RAISE NOTICE 'Table Zf existante pour le territoire ''%''. Poursuite du traitement', nom_ter;
		EXECUTE '
			...
			FROM c_phenomenes.zf AS zf
			WHERE __util_to_snake_case(zf.territoire) = ''' || __util_to_snake_case(nom_ter) || '''
				AND __util_to_snake_case(s2_2a.territoire) = ''' || __util_to_snake_case(nom_ter) || '''
				AND __util_to_snake_case(zf.type_alea) = ''' || __util_to_snake_case(typ_alea) || '''
				AND __util_to_snake_case(s2_2a.type_alea) = ''' || __util_to_snake_case(typ_alea) || '''
				AND zf.code_occurrence = ''' || code_occ || '''
	...
	RAISE NOTICE 'Aucune couche Zf n''est disponible pour le territoire ''%'' dans le schéma c_phenomenes. Attribution d''une durée de submersion arbitraire en fonction de la nature de l''aléa renseigné', nom_ter;
		EXECUTE '
			...
			SET duree_subm = (
			CASE
				WHEN s2_2a.type_alea_fct_dmg = ''fluvial'' AND s2_2a.code_occurrence LIKE ''%2'' THEN ''<48h''
				WHEN s2_2a.type_alea_fct_dmg = ''fluvial'' AND s2_2a.code_occurrence LIKE ''%5'' THEN ''<48h''
				WHEN s2_2a.type_alea_fct_dmg = ''fluvial'' AND s2_2a.code_occurrence LIKE ''%10'' THEN ''<48h''
				WHEN s2_2a.type_alea_fct_dmg = ''fluvial'' AND s2_2a.code_occurrence LIKE ''%20'' THEN ''<48h''
				WHEN s2_2a.type_alea_fct_dmg = ''fluvial'' AND s2_2a.code_occurrence LIKE ''%30'' THEN ''<48h''
				ELSE ''>48h''
			END)
			WHERE __util_to_snake_case(s2_2a.territoire) = ''' || __util_to_snake_case(nom_ter) || ''' AND __util_to_snake_case(s2_2a.type_alea) = ''' || __util_to_snake_case(typ_alea) || ''' AND s2_2a.code_occurrence = ''' || code_occ || ''' AND s2_2a.date_actu_cout_dmg = '''|| an_fct_dmg ||'''';
    ...
    --************************************************************************
	-- ÉTAPE 5A : CALCUL DU COÛT DES DOMMAGES AUX STRUCTURES
	--************************************************************************
	RAISE NOTICE '';
	RAISE NOTICE 'Etape 5a : calcul du coût des dommages au bâti (structure) pour chaque logement identifié à l''étape 1, en fonction du type d''aléa, de la durée de submersion et de la hauteur d''eau';

	-- Dommages au bâti / Aléa fluvial / Durée de submersion < 48 heures
	-- H(Zx)min
	EXECUTE '
		UPDATE p_indicateurs.s2_2a AS s2_2a
		SET cout_min_dmg_bati = (
			CASE
				WHEN s2_2a.type_alea_fct_dmg = ''fluvial'' AND s2_2a.typo_acb = ''INDIVIDUEL SANS ETAGE'' AND s2_2a.duree_subm = ''<48h'' THEN ROUND(s2_2a.surf_rdc_rect_alea * fct.individuel_sans_etage)
				WHEN s2_2a.type_alea_fct_dmg = ''fluvial'' AND s2_2a.typo_acb = ''INDIVIDUEL AVEC ETAGE'' AND s2_2a.duree_subm = ''<48h'' THEN ROUND(s2_2a.surf_rdc_rect_alea * fct.individuel_avec_etage)
				WHEN s2_2a.type_alea_fct_dmg = ''fluvial'' AND s2_2a.typo_acb = ''COLLECTIF'' AND s2_2a.duree_subm = ''<48h'' THEN ROUND(s2_2a.surf_rdc_rect_alea * fct.logement_collectif)
		...
		WHERE
			...
			AND fct.duree_submersion = ''<48h''
			AND s2_2a.loc_zx = ''In''
			AND s2_2a.h_eau_min >= fct.h_eau_min
			AND s2_2a.h_eau_min <= fct.h_eau_max
			AND __util_to_snake_case(s2_2a.territoire) = ''' || __util_to_snake_case(nom_ter) || '''
			AND __util_to_snake_case(s2_2a.type_alea) = ''' || __util_to_snake_case(typ_alea) || '''
			AND s2_2a.code_occurrence = ''' || code_occ || '''
			AND s2_2a.date_actu_cout_dmg = '''|| an_fct_dmg ||'''';
		...
		WHERE
			CASE
				WHEN
					...
					AND fct.duree_submersion = ''<48h''
					AND s2_2a.loc_zx = ''In''
					AND s2_2a.h_eau_max < 999
					AND __util_to_snake_case(s2_2a.territoire) = ''' || __util_to_snake_case(nom_ter) || '''
					AND __util_to_snake_case(s2_2a.type_alea) = ''' || __util_to_snake_case(typ_alea) || '''
					AND s2_2a.code_occurrence = ''' || code_occ || '''
					AND s2_2a.date_actu_cout_dmg = '''|| an_fct_dmg ||'''
				THEN
					s2_2a.h_eau_max >= fct.h_eau_min
					AND s2_2a.h_eau_max <= fct.h_eau_max
				ELSE
					fct.alea = ''fluvial''
					AND fct.unite_cout = ''surfacique''
					AND fct.duree_submersion = ''<48h''
					AND fct.h_eau_max = 305
					AND s2_2a.loc_zx = ''In''
					AND __util_to_snake_case(s2_2a.territoire) = ''' || __util_to_snake_case(nom_ter) || '''
					AND __util_to_snake_case(s2_2a.type_alea) = ''' || __util_to_snake_case(typ_alea) || '''
					AND s2_2a.code_occurrence = ''' || code_occ || '''
	...
	-- Dommages au bâti / Aléa submersion marine / Durée de submersion < 48 heures
	-- H(Zx)min
	EXECUTE '
		UPDATE p_indicateurs.s2_2a AS s2_2a
		SET cout_min_dmg_bati = (
			CASE
				WHEN s2_2a.type_alea_fct_dmg = ''marin'' AND s2_2a.typo_acb = ''INDIVIDUEL SANS ETAGE'' AND s2_2a.duree_subm = ''<48h'' THEN ROUND(s2_2a.surf_rdc_rect_alea * fct.individuel_sans_etage)
				WHEN s2_2a.type_alea_fct_dmg = ''marin'' AND s2_2a.typo_acb = ''INDIVIDUEL AVEC ETAGE'' AND s2_2a.duree_subm = ''<48h'' THEN ROUND(s2_2a.surf_rdc_rect_alea * fct.individuel_avec_etage)
				WHEN s2_2a.type_alea_fct_dmg = ''marin'' AND s2_2a.typo_acb = ''COLLECTIF'' AND s2_2a.duree_subm = ''<48h'' THEN ROUND(s2_2a.surf_rdc_rect_alea * fct.logement_collectif)
				ELSE cout_min_dmg_bati
			END)
		FROM r_inrae_amc.fct_dmg_logts_' || an_fct_dmg || ' AS fct
		WHERE
    ...
    --************************************************************************
	-- ÉTAPE 5B : CALCUL DU COÛT DES DOMMAGES AU MOBILIER
	--************************************************************************
	RAISE NOTICE 'Etape 5b : calcul du coût des dommages au mobilier pour chaque logement identifié à l''étape 1, en fonction du type d''aléa, de la durée de submersion et de la hauteur d''eau';

	-- Dommages au mobilier / Aléa fluvial / Durée de submersion < 48 heures
	-- H(Zx)min
	EXECUTE '
		UPDATE p_indicateurs.s2_2a AS s2_2a
		SET cout_min_dmg_mob = (
			CASE
				WHEN s2_2a.type_alea_fct_dmg = ''fluvial'' AND s2_2a.typo_acb = ''INDIVIDUEL SANS ETAGE'' AND s2_2a.duree_subm = ''<48h'' THEN ROUND(s2_2a.surf_rdc_rect_alea * fct.mobilier_individuel_sans_etage)
				WHEN s2_2a.type_alea_fct_dmg = ''fluvial'' AND s2_2a.typo_acb = ''INDIVIDUEL AVEC ETAGE'' AND s2_2a.duree_subm = ''<48h'' THEN ROUND(s2_2a.surf_rdc_rect_alea * fct.mobilier_individuel_avec_etage)
				WHEN s2_2a.type_alea_fct_dmg = ''fluvial'' AND s2_2a.typo_acb = ''COLLECTIF'' AND s2_2a.duree_subm = ''<48h'' THEN ROUND(s2_2a.surf_rdc_rect_alea * fct.mobilier_logement_collectif)
	...
	-- Dommages au mobilier / Aléa submersion marine / Durée de submersion < 48 heures
	-- H(Zx)min
	EXECUTE '
		UPDATE p_indicateurs.s2_2a AS s2_2a
		SET cout_min_dmg_mob = (
			CASE
				WHEN s2_2a.type_alea_fct_dmg = ''marin'' AND s2_2a.typo_acb = ''INDIVIDUEL SANS ETAGE'' AND s2_2a.duree_subm = ''<48h'' THEN ROUND(s2_2a.surf_rdc_rect_alea * fct.mobilier_individuel_sans_etage)
				WHEN s2_2a.type_alea_fct_dmg = ''marin'' AND s2_2a.typo_acb = ''INDIVIDUEL AVEC ETAGE'' AND s2_2a.duree_subm = ''<48h'' THEN ROUND(s2_2a.surf_rdc_rect_alea * fct.mobilier_individuel_avec_etage)
				WHEN s2_2a.type_alea_fct_dmg = ''marin'' AND s2_2a.typo_acb = ''COLLECTIF'' AND s2_2a.duree_subm = ''<48h'' THEN ROUND(s2_2a.surf_rdc_rect_alea * fct.mobilier_logement_collectif)
				ELSE cout_min_dmg_mob
			END)
		FROM r_inrae_amc.fct_dmg_logts_' || an_fct_dmg || ' AS fct
		WHERE
    ...
    --************************************************************************
	-- ÉTAPE 5C : CALCUL DU COÛT DES DOMMAGES AUX SOUS-SOLS
	--************************************************************************
	RAISE NOTICE 'Etape 5c : calcul du coût des dommages aux sous-sols ou caves pour chaque logement identifié à l''étape 1, en fonction du type d''aléa, de la durée de submersion et de la hauteur d''eau';

	-- Dommages aux sous-sols / Aléa fluvial / Durée de submersion < 48 heures
	-- H(Zx)min
	EXECUTE '
		UPDATE p_indicateurs.s2_2a AS s2_2a
		SET cout_min_dmg_ssol = (
			CASE
				WHEN s2_2a.type_alea_fct_dmg = ''fluvial'' AND s2_2a.type_logt = ''MAISON'' AND s2_2a.duree_subm = ''<48h'' THEN ROUND(s2_2a.surf_ssol_alea * fct.sous_sol_individuel)
				WHEN s2_2a.type_alea_fct_dmg = ''fluvial'' AND s2_2a.type_logt = ''APPARTEMENT'' AND s2_2a.duree_subm = ''<48h'' THEN ROUND(s2_2a.surf_ssol_alea * fct.sous_sol_collectif)
	...
	-- Dommages aux sous-sols / Aléa submersion marine / Durée de submersion < 48 heures
	-- H(Zx)min
	EXECUTE '
		UPDATE p_indicateurs.s2_2a AS s2_2a
		SET cout_min_dmg_ssol = (
			CASE
				WHEN s2_2a.type_alea_fct_dmg = ''marin'' AND s2_2a.type_logt = ''MAISON'' AND s2_2a.duree_subm = ''<48h'' THEN ROUND(s2_2a.surf_ssol_alea * fct.sous_sol_individuel)
				WHEN s2_2a.type_alea_fct_dmg = ''marin'' AND s2_2a.type_logt = ''APPARTEMENT'' AND s2_2a.duree_subm = ''<48h'' THEN ROUND(s2_2a.surf_ssol_alea * fct.sous_sol_collectif)
				ELSE cout_min_dmg_ssol
			END)
		FROM r_inrae_amc.fct_dmg_logts_' || an_fct_dmg || ' AS fct
		WHERE
    ...
    --************************************************************************
	-- ÉTAPE 5D : CALCUL DU COÛT TOTAL DES DOMMAGES
	--************************************************************************
	RAISE NOTICE 'Etape 5d : calcul du coût total des dommages pour chaque logement identifié à l''étape 1, en fonction du type d''aléa, de la durée de submersion et de la hauteur d''eau';
	
	EXECUTE '
		UPDATE p_indicateurs.s2_2a
		SET cout_min_dmg_tot = cout_min_dmg_bati + cout_min_dmg_mob + cout_min_dmg_ssol
		WHERE s2_2a.loc_zx = ''In''
			AND __util_to_snake_case(s2_2a.territoire) = ''' || __util_to_snake_case(nom_ter) || '''
			AND __util_to_snake_case(s2_2a.type_alea) = ''' || __util_to_snake_case(typ_alea) || '''
			AND s2_2a.code_occurrence = ''' || code_occ || '''
			AND s2_2a.date_actu_cout_dmg = '''|| an_fct_dmg ||'''';
	...
	EXECUTE '
		UPDATE p_indicateurs.s2_2a
		SET cout_max_dmg_tot = cout_max_dmg_bati + cout_max_dmg_mob + cout_max_dmg_ssol
		WHERE s2_2a.loc_zx = ''In''
			AND __util_to_snake_case(s2_2a.territoire) = ''' || __util_to_snake_case(nom_ter) || '''
			AND __util_to_snake_case(s2_2a.type_alea) = ''' || __util_to_snake_case(typ_alea) || '''
			AND s2_2a.code_occurrence = ''' || code_occ || '''
	...
	RAISE NOTICE '[INFO] La table s2_2a (montant des dommages aux logements en €%) a été mise à jour dans le schéma p_indicateurs pour le territoire ''%'' et l''aléa ''%'' - %', an_fct_dmg, nom_ter, typ_alea, code_occ;
	RAISE NOTICE '';
	RAISE NOTICE '====== RESULTATS ======';
	RAISE NOTICE '% entités ajoutées pour le territoire ''%'' dans p_indicateurs.s2_2a', c, nom_ter;
	-- RAISE NOTICE 'Création de la vue "%"', 'p_indicateurs.s2_2a_' || __util_to_snake_case(nom_ter);
