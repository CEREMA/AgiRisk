...
CREATE OR REPLACE FUNCTION public.__indic_s2_2a_amc(nom_ter text, typ_alea text, code_occ text, an_fct_dmg text, an_bdtopo text)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
...
-- © Cerema / GT AgiRisk (auteur principal du script, inspiré de ceux mis en œuvre localement par d'autres membres du GT AgiRisk : Sébastien et optimisation par Lucie)
-- Dernières mises à jour du script le 22/05/2023 par Lucie et le 17/09/2023 par Sébastien
...
-- SELECT public.__indic_s2_2a_amc('TRI Verdun', 'débordement de cours d''eau + remontée de nappe', 'QRef diag', '2022', '2022');
...
* onglet "surfacique (€2016/m²)" = fonctions de dommages surfaciques :
	...
	-> Fonctions utilisables avec les fichiers AMC produits par Christophe.
...
BEGIN
	...
	RAISE NOTICE '[NOTA] Calcul basé sur les variables oc2_amc et les couches d''aléas disponibles dans la base de données';
	...
	--************************************************************************
	-- Vérification de l'absence de données pour le territoire, le type d'aléa, l'occurrence de crue et l'année d'actualisation des fonctions de dommages renseignés dans la table p_indicateurs.s2_2a_amc
	--************************************************************************
	IF EXISTS(
		SELECT *
	...
	THEN
		RAISE EXCEPTION 'Il existe déjà des enregistrements en euros % pour le territoire "%" et l''aléa "% - %" dans la table p_indicateurs.s2_2a_amc. 
		Pour les supprimer, lancer la requête suivante : 
		DELETE FROM p_indicateurs.s2_2a_amc WHERE territoire = ''%'' AND type_alea = ''%'' AND code_occurrence = ''%'' AND date_actu_cout_dmg = ''%'';', an_fct_dmg, nom_ter, typ_alea, code_occ, REPLACE(nom_ter,'''',''''''), REPLACE(typ_alea,'''',''''''), REPLACE(code_occ,'''',''''''), REPLACE(an_fct_dmg,'''','''''');
	END IF;
	...
	RAISE NOTICE 'Etape 1a : intersection de la couche des logements avec Zh (classes de hauteur d''eau)';
		...
		EXECUTE 'INSERT INTO p_indicateurs.s2_2a_amc(
					territoire,
					loc_zx,
					type_alea,
					code_occurrence,
				...
				SELECT
					...
					oc2.sce_donnee,
					''__indic_s2_2a_amc''::varchar(50) AS moda_calc,
		...
		EXECUTE '
			UPDATE p_indicateurs.s2_2a_amc
		...
		EXECUTE '
			...
			SET intensite_alea = (
				CASE
					...
					WHEN h_eau_min >= 0 AND h_eau_max > 0 AND h_eau_max <= 50 THEN ''faible''
					WHEN h_eau_min >= 0 AND h_eau_max > 50 AND h_eau_max <= 100 THEN ''moyen''
					WHEN h_eau_min >= 0 AND h_eau_max > 100 AND h_eau_max <= 200 THEN ''fort''
					WHEN h_eau_min >= 0 AND h_eau_max > 200 THEN ''très fort''
					WHEN h_eau_min >= 50 AND h_eau_max <= 100 THEN ''moyen''
					WHEN h_eau_min >= 100 AND h_eau_max <= 200 THEN ''fort''
					WHEN h_eau_min >= 100 AND h_eau_min < 200 AND h_eau_max > 200 THEN ''fort''
					WHEN h_eau_min >= 200 AND h_eau_max <= 999 THEN ''très fort''
					ELSE intensite_alea
			END)
			WHERE __util_to_snake_case(territoire) = '''||__util_to_snake_case(nom_ter)||'''
	...
	-- a. Récupération des codes et noms des IRIS
		EXECUTE '
			...
			WHERE s2_2a_amc.date_actu_cout_dmg = '''||an_fct_dmg||'''
				...
				AND ST_Intersects(s2_2a_amc.geom, zt.geom)';
	...
	-- b. Attribution des codes INSEE et noms de communes
		EXECUTE '
			...
			WHERE s2_2a_amc.date_actu_cout_dmg = '''||an_fct_dmg||'''
				...
				AND __util_to_snake_case(s2_2a_amc.code_occurrence) = '''||__util_to_snake_case(code_occ)||'''
				AND __util_to_snake_case(zt.territoire) = '''||__util_to_snake_case(nom_ter)||'''
				AND s2_2a_amc.id_iris = zt.id_iris
	...
	RAISE NOTICE 'Table Zf existante pour le territoire "%". Poursuite du traitement', nom_ter;
		EXECUTE '
			...
			WHERE __util_to_snake_case(zf.territoire) = ''' || __util_to_snake_case(nom_ter) || '''
				AND __util_to_snake_case(s2_2a_amc.territoire) = __util_to_snake_case(zf.territoire)
				AND __util_to_snake_case(zf.type_alea) = ''' || __util_to_snake_case(typ_alea) || '''
				AND __util_to_snake_case(s2_2a_amc.type_alea) = __util_to_snake_case(zf.type_alea)
				AND __util_to_snake_case(zf.code_occurrence) = ''' || __util_to_snake_case(code_occ) || '''
				AND __util_to_snake_case(s2_2a_amc.code_occurrence) = __util_to_snake_case(zf.code_occurrence)
				AND s2_2a_amc.date_actu_cout_dmg = '''|| an_fct_dmg ||'''';
	...
	EXECUTE '
		...
		SET 
			...
			cout_min_dmg_bati =
			CASE
				WHEN s2_2a_amc.typo_acb = ''INDIVIDUEL SANS ETAGE'' THEN ROUND(s2_2a_amc.surf_rdc_rect_alea * fct.individuel_sans_etage)
				WHEN s2_2a_amc.typo_acb = ''INDIVIDUEL AVEC ETAGE'' THEN ROUND(s2_2a_amc.surf_rdc_rect_alea * fct.individuel_avec_etage)
				WHEN s2_2a_amc.typo_acb = ''COLLECTIF'' THEN ROUND(s2_2a_amc.surf_rdc_rect_alea * fct.logement_collectif)
			...
			cout_min_dmg_mob =
			CASE
				WHEN s2_2a_amc.typo_acb = ''INDIVIDUEL SANS ETAGE'' THEN ROUND(s2_2a_amc.surf_rdc_rect_alea * fct.mobilier_individuel_sans_etage)
				WHEN s2_2a_amc.typo_acb = ''INDIVIDUEL AVEC ETAGE'' THEN ROUND(s2_2a_amc.surf_rdc_rect_alea * fct.mobilier_individuel_avec_etage)
				WHEN s2_2a_amc.typo_acb = ''COLLECTIF'' THEN ROUND(s2_2a_amc.surf_rdc_rect_alea * fct.mobilier_logement_collectif)
			...
			cout_min_dmg_ssol =
			CASE
				WHEN s2_2a_amc.type_logt = ''MAISON'' THEN ROUND(s2_2a_amc.surf_ssol_alea * fct.sous_sol_individuel)
				WHEN s2_2a_amc.type_logt = ''APPARTEMENT'' THEN ROUND(s2_2a_amc.surf_ssol_alea * fct.sous_sol_collectif)
		...
		WHERE
			...
			AND fct.alea = s2_2a_amc.type_alea_fct_dmg
			AND fct.duree_submersion IN (''<48h'',''>48h'')
			AND fct.duree_submersion = s2_2a_amc.duree_subm
			AND fct.unite_cout = ''surfacique''
			AND s2_2a_amc.loc_zx = ''In''
			AND
				(
					(s2_2a_amc.h_eau_min <= 305 AND s2_2a_amc.h_eau_min >= fct.h_eau_min AND s2_2a_amc.h_eau_min <= fct.h_eau_max)
				OR
					(s2_2a_amc.h_eau_min > 305 AND fct.h_eau_max = 305)
				)
			AND __util_to_snake_case(s2_2a_amc.territoire) = ''' || __util_to_snake_case(nom_ter) || '''
			AND __util_to_snake_case(s2_2a_amc.type_alea) = ''' || __util_to_snake_case(typ_alea) || '''
			AND __util_to_snake_case(s2_2a_amc.code_occurrence) = ''' || __util_to_snake_case(code_occ) || '''
	...
	EXECUTE '
		...
		WHERE
			...
			AND
				(
					(s2_2a_amc.h_eau_max <= 305 AND s2_2a_amc.h_eau_max >= fct.h_eau_min AND s2_2a_amc.h_eau_max <= fct.h_eau_max)
				OR
					(s2_2a_amc.h_eau_max > 305 AND fct.h_eau_max = 305)
				)
			AND __util_to_snake_case(s2_2a_amc.territoire) = ''' || __util_to_snake_case(nom_ter) || '''
			AND __util_to_snake_case(s2_2a_amc.type_alea) = ''' || __util_to_snake_case(typ_alea) || '''
			AND __util_to_snake_case(s2_2a_amc.code_occurrence) = ''' || __util_to_snake_case(code_occ) || '''
	...
	RAISE NOTICE '[INFO] La table s2_2a_amc (montant des dommages aux logements en €%) a été mise à jour dans le schéma p_indicateurs pour le territoire "%" et l''aléa "% - %"', an_fct_dmg, nom_ter, typ_alea, code_occ;
	RAISE NOTICE '';
	RAISE NOTICE '====== RÉSULTATS ======';

	IF c > 1
		THEN RAISE NOTICE '% enregistrements insérés au total dans la table p_indicateurs.s2_2a_amc pour le territoire "%" et l''aléa "% - %"', c, nom_ter, typ_alea, code_occ;
		ELSE RAISE NOTICE '% enregistrement inséré dans la table p_indicateurs.s2_2a_amc pour le territoire "%" et l''aléa "% - %"', c, nom_ter, typ_alea, code_occ;
	END IF;
