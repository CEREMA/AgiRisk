...
-- Fonction SQL d'incrémentation de la table p_indicateurs.pop_agee_zx "Population âgée de plus de 65 ans en zone inondable"
...
-- SELECT public.__indic_pop_agee_zx('Jura', 'débordement de cours d''eau', 'QRef', '2022');
...
BEGIN
	...
	RAISE NOTICE 'Calcul de l''indicateur pop_agee_zx (nombre et proportion d''habitants âgés de plus de 65 ans en zone inondable) sur le territoire "%" pour l''aléa %', nom_ter, code_occ;
	...
	--************************************************************************
	-- Vérification de l'absence de données pour le territoire, le type d'aléa et l'occurrence de crue renseignés dans la table p_indicateurs.pop_agee_zx
	--************************************************************************
	IF EXISTS (
		SELECT *
	...
	THEN
		RAISE EXCEPTION 'Il existe déjà des enregistrements pour le territoire "%" avec l''aléa "% - %" dans la table p_indicateurs.pop_agee_zx. 
		Pour les supprimer, lancer la requête suivante : 
		DELETE FROM p_indicateurs.pop_agee_zx WHERE territoire = ''%'' AND type_alea = ''%'' AND code_occurrence = ''%'';', nom_ter, typ_alea, code_occ, REPLACE(nom_ter,'''',''''''), REPLACE(typ_alea,'''',''''''), REPLACE(code_occ,'''','''''');
	END IF;
	...
	RAISE NOTICE 'Etape 1a : récupération des bâtiments inondables avec résidents âgés de plus de 65 ans sur le territoire "%" pour l''aléa "% - %"', nom_ter, typ_alea, code_occ;
	EXECUTE 'INSERT INTO p_indicateurs.pop_agee_zx (
			territoire,
			id_iris,
			nom_iris,
			id_bdt,
	...
	GET DIAGNOSTICS c_in = row_count; -- nombre de bâtiments avec résidents âgés de plus de 65 ans en zone inondable insérés dans la table

	-- 1b. Bâtiments hors zone inondable (Out)
	RAISE NOTICE 'Etape 1b : insertion des bâtiments avec résidents âgés de plus de 65 ans qui se trouvent en dehors de la zone inondable (Zx) sur le territoire "%" pour l''aléa "% - %"', nom_ter, typ_alea, code_occ;
	EXECUTE 'INSERT INTO p_indicateurs.pop_agee_zx (
			territoire,
			id_iris,
			nom_iris,
	...
	-- Récupération du nombre d'entités ajoutées
	GET DIAGNOSTICS c_out = row_count; -- nombre de bâtiments avec résidents âgés de plus de 65 ans hors zone inondable insérés dans la table
	...
	-- 2a. Attribution des codes INSEE et noms de communes
		EXECUTE '
			UPDATE p_indicateurs.pop_agee_zx
			...
			WHERE __util_to_snake_case(pop_agee_zx.territoire) = '''||__util_to_snake_case(nom_ter)||'''
				...
				AND pop_agee_zx.id_iris = zt.id_iris
	...
	RAISE NOTICE '[INFO] La table pop_agee_zx (habitants âgés de plus de 65 ans en zone inondable) a été mise à jour dans le schéma p_indicateurs pour le territoire "%" et l''aléa "% - %"', nom_ter, typ_alea, code_occ;
	RAISE NOTICE '';
	RAISE NOTICE '====== RÉSULTATS ======';

	IF c_in+c_out > 1
		THEN RAISE NOTICE '% enregistrements insérés au total dans la table p_indicateurs.pop_agee_zx pour le territoire "%" et l''aléa "% - %"', c_in+c_out, nom_ter, typ_alea, code_occ;
		ELSE RAISE NOTICE '% enregistrement inséré dans la table p_indicateurs.pop_agee_zx pour le territoire "%" et l''aléa "% - %"', c_in+c_out, nom_ter, typ_alea, code_occ;
	END IF;

	RAISE NOTICE 'dont :';
	
		IF c_in > 1
			THEN RAISE NOTICE '- % bâtiments avec résidents âgés de plus de 65 ans en zone inondable', c_in;
			ELSE RAISE NOTICE '- % bâtiment avec résidents âgés de plus de 65 ans en zone inondable', c_in;
		END IF;
	
		IF c_out > 1
			THEN RAISE NOTICE '- % bâtiments avec résidents âgés de plus de 65 ans hors zone inondable', c_out;
			ELSE RAISE NOTICE '- % bâtiment avec résidents âgés de plus de 65 ans hors zone inondable', c_out;
		END IF;
