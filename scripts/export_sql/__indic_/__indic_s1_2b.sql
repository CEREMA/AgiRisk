...
CREATE OR REPLACE FUNCTION public.__indic_s1_2b(nom_ter text, typ_alea text, code_occ text, an_bdtopo text)
 RETURNS void
 LANGUAGE plpgsql
AS $function$

-- Fonction SQL d'incrémentation de la table p_indicateurs.s1_2b "Personnes occupant des locaux fortement inondables"
...
-- SELECT public.__indic_s1_2b('Jura', 'débordement de cours d''eau', 'QRef', '2022');
...
BEGIN
	...
	RAISE NOTICE 'Calcul de l''indicateur S1/2b (occupants en rez-de-chaussée de bâtiments) sur le territoire "%" pour l''aléa "% - %"', nom_ter, typ_alea, code_occ;
	...
	--************************************************************************
	-- Vérification de l'absence de données pour le territoire, le type d'aléa et l'occurrence de crue renseignés dans la table p_indicateurs.s1_2b
	--************************************************************************
	IF EXISTS (
		SELECT *
	...
	THEN
		RAISE EXCEPTION 'Il existe déjà des enregistrements pour le territoire "%" avec l''aléa "% - %" dans la table p_indicateurs.s1_2b. 
		Pour les supprimer, lancer la requête suivante : 
		DELETE FROM p_indicateurs.s1_2b WHERE territoire = ''%'' AND type_alea = ''%'' AND code_occurrence = ''%'';', nom_ter, typ_alea, code_occ, REPLACE(nom_ter,'''',''''''), REPLACE(typ_alea,'''',''''''), REPLACE(code_occ,'''','''''');
	END IF;

	--************************************************************************
	-- ÉTAPE 1 : Croisement des bâtiments (Oc1) avec les zones fortement inondables sur le territoire d'étude
	--************************************************************************
	RAISE NOTICE '';
	...
	-- INFO : 03/10/2022 - Pas de champ pop6_bas considéré ... pop6_rdc = pop6_haut

	-- a) Insertion des bâtiments oc1 qui intersectent une zone d''aléa très fort
	EXECUTE 'INSERT INTO p_indicateurs.s1_2b (
			territoire,
			id_iris,
			nom_iris,
			id_bdt,
		...
		SELECT DISTINCT
			...
			zq.type_alea,
			zq.code_occurrence,
			oc1.pop6_haut::numeric/nb_etages_corr::numeric,
			oc1.sce_donnee,
			''__indic_s1_2b'',
			current_date,
			oc1.geom,
	...
	GET DIAGNOSTICS c_in_tres_fort = row_count; -- nombre de bâtiments en zone d'aléa très fort insérés dans la table
	...
	EXECUTE 'INSERT INTO p_indicateurs.s1_2b (
		...
		SELECT DISTINCT
			...
			zq.type_alea,
			zq.code_occurrence,
			oc1.pop6_haut::numeric/nb_etages_corr,
			oc1.sce_donnee,
			''__indic_s1_2b'',
			current_date,
			oc1.geom,
	...
	-- Récupération du nombre d'entités ajoutées
	GET DIAGNOSTICS c_in_fort = row_count; -- nombre de bâtiments en zone d'aléa fort insérés dans la table
	...
	-- Récupération du nombre d'entités ajoutées
	GET DIAGNOSTICS c_in_moyen = row_count; -- nombre de bâtiments en zone d'aléa moyen insérés dans la table
	...
	-- Récupération du nombre d'entités ajoutées
	GET DIAGNOSTICS c_in_faible = row_count; -- nombre de bâtiments en zone d'aléa faible insérés dans la table
	...
	RAISE NOTICE '[INFO] La table s1_2b (occupants en rez-de-chaussée) a été mise à jour dans le schéma p_indicateurs pour le territoire "%" et l''aléa "% - %"', nom_ter, typ_alea, code_occ;
	RAISE NOTICE '';
	RAISE NOTICE '====== RÉSULTATS ======';

	IF c_in_tres_fort+c_in_fort+c_in_moyen+c_in_faible+c_out > 1
		THEN RAISE NOTICE '% enregistrements insérés au total dans la table p_indicateurs.s1_2b pour le territoire "%" et l''aléa "% - %"', c_in_tres_fort+c_in_fort+c_in_moyen+c_in_faible+c_out, nom_ter, typ_alea, code_occ;
		ELSE RAISE NOTICE '% enregistrement inséré dans la table p_indicateurs.s1_2b pour le territoire "%" et l''aléa "% - %"', c_in_tres_fort+c_in_fort+c_in_moyen+c_in_faible+c_out, nom_ter, typ_alea, code_occ;
	END IF;

	RAISE NOTICE 'dont :';
	
		IF c_in_tres_fort > 1
			THEN RAISE NOTICE '- % bâtiments en zone d''aléa très fort', c_in_tres_fort;
			ELSE RAISE NOTICE '- % bâtiment en zone d''aléa très fort', c_in_tres_fort;
		END IF;
	
		IF c_in_fort > 1
			THEN RAISE NOTICE '- % bâtiments en zone d''aléa fort', c_in_fort;
			ELSE RAISE NOTICE '- % bâtiment en zone d''aléa fort', c_in_fort;
		END IF;
	
		IF c_in_moyen > 1
			THEN RAISE NOTICE '- % bâtiments en zone d''aléa moyen', c_in_moyen;
			ELSE RAISE NOTICE '- % bâtiment en zone d''aléa moyen', c_in_moyen;
		END IF;
	
		IF c_in_faible > 1
			THEN RAISE NOTICE '- % bâtiments en zone d''aléa faible', c_in_faible;
			ELSE RAISE NOTICE '- % bâtiment en zone d''aléa faible', c_in_faible;
		END IF;
	
		IF c_out > 1
			THEN RAISE NOTICE '- % bâtiments hors zone inondable', c_out;
			ELSE RAISE NOTICE '- % bâtiment hors zone inondable', c_out;
		END IF;
