...
CREATE OR REPLACE FUNCTION public.__var_oc1_geomloc(nom_ter text, an_bdtopo text)
 RETURNS void
 LANGUAGE plpgsql
AS $function$

-- Modalité de calcul d'Oc1 basée sur une intersection des centroïdes des polygones des bâtiments (champ geomloc de la table batiment de la BD TOPO®) avec le périmètre du territoire d'étude

-- Fonction SQL d'incrémentation de la table oc1 (bâtiments) dans le schéma c_occupation_sol
-- © Cerema / GT AgiRisk
-- Dernière mise à jour du script le 25/08/2023 par Sébastien  puis le 26/09/2023 par Aurélien
...
-- SELECT public.__var_oc1_geomloc('Jura', '2022');
...
BEGIN
	...
	--************************************************************************
	-- Calcul Oc1 sur la base d'une insertion des centroïdes des polygones (geomloc) avec le périmètre du territoire donné
	--************************************************************************
	RAISE NOTICE '';
