...
CREATE OR REPLACE FUNCTION public.__var_pop1_pas_a_pas(nom_ter text, an_ff text, an_pop text)
 RETURNS void
 LANGUAGE plpgsql
AS $function$
...
-- © Cerema / GT AgiRisk
-- Dernière mise à jour du script le 19/07/2023 par Sébastien
...
-- Exemple d'appel à cette fonction : SELECT public.__var_pop1('Jura', '2021', '2015');
...
--	--************************************************************************
    EXECUTE '
		...
		WHERE __util_to_snake_case(oc1.territoire) = '''||__util_to_snake_case(nom_ter)||'''
			AND ST_Intersects(oc1.geomloc,tup.geomtup)';
--
--	--************************************************************************
...
--    EXECUTE '
--		UPDATE temp_oc1
--		SET id_popref = i.idinspire, nb_popref = i.ind, nb_popref_agee = (i.ind_65_79 + i.ind_80p)
...
--
--	--************************************************************************
--	-- Requêtes complétant les champs sce_donnee et moda_calc de la table oc1
--    --************************************************************************
--	EXECUTE '
--		UPDATE c_occupation_sol.oc1
--		SET sce_donnee = sce_donnee || '', Filosofi INSEE '||an_pop||'''
--		WHERE __util_to_snake_case(oc1.territoire) = '''||__util_to_snake_case(nom_ter)||'''';
--
--	EXECUTE '
--		UPDATE c_occupation_sol.oc1
--		SET moda_calc = moda_calc || '', __var_pop1''
--		WHERE __util_to_snake_case(oc1.territoire) = '''||__util_to_snake_case(nom_ter)||'''';
--
--	--************************************************************************
...
--	ON c_occupation_sol.oc1_'||__util_to_snake_case(nom_ter)||' (id)';
--
